...
export class SandboxImageCarousel extends TailwindElement {
  ...
  private renderImage (source: HTMLImage, index: number): TemplateResult {
    ...
    return html`
      <div class="aspect-[9/10] w-44 flex-none overflow-hidden rounded-xl bg-zinc-100 dark:bg-zinc-800 sm:w-72 sm:rounded-2xl ${cssRotateClass}">
        <img alt="${source.alt}" loading="lazy" decoding="async"
             src="${source.src}"
