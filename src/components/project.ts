...
export class SandboxProjects extends TailwindElement {
  protected override render (): TemplateResult {
    return html`
      <div class="flex flex-col gap-16"><slot></slot></div>
    `
  }
}
    ...
    return html`
      <div class="group relative flex flex-col items-start">
        ...
        <h2 class="mt-6 text-base font-semibold text-zinc-800 dark:text-zinc-100">
          ...
          <a href="${this.link?.href}"><span
              class="absolute -inset-x-4 -inset-y-6 z-20 sm:-inset-x-6 sm:rounded-2xl"></span><span
              class="relative z-10">${this.title}</span></a></h2>
        <p class="relative z-10 mt-2 text-sm text-zinc-600 dark:text-zinc-400">
          <slot></slot>
        </p>
        <p class="relative z-10 mt-6 flex text-sm font-medium text-zinc-400 
        transition group-hover:text-blue-500 dark:text-zinc-200">
          ...
          <span class="ml-2">${this.link?.text}</span></p>
      </div>
    `
  }
}
