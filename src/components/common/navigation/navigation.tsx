...
export const Navigation = (props: PropsWithChildren<Props>): ReactElement => {
  ...
  function _renderLinks (links: HTMLSimpleLink[]): ReactElement {
    return <ul className={'flex rounded-full bg-white/90 px-3 text-sm font-medium text-zinc-800 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10'}>
        {links.map((link, i) => {
  ...
  return <div className={'flex justify-between md:justify-center flex-row mt-8 px-4 md:px-2 items-center'}>
      ...
      <div className={'md:basis-1/3'}>{_renderLinks(props.links)}</div>
