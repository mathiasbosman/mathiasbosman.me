...
export const ImageCarousel = (props: PropsWithChildren<Props>): ReactElement => {
  const { random = false, amountToShow = 5, images } = props;
  const imageSet: HTMLImage[] = random ? shuffleArray(images) : images;
    ...
    <div className="mt-8 flex justify-center gap-4 overflow-hidden py-4 sm:mt-16 sm:gap-8">
      {imageSet.slice(0, amountToShow).map((img, i) => {
        ...
        return (
          <div
            key={i}
            className={`${cssRotation} relative aspect-[5/10] w-24 flex-none overflow-hidden rounded-xl bg-zinc-100 sm:w-36 sm:rounded-2xl lg:aspect-[9/10] lg:w-72 dark:bg-zinc-800`}
          >
            <img
              alt={img.alt}
              src={img.src}
